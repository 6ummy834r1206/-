...
public class DungeonGenerator : MonoBehaviour {
    ...
    public int targetBossRooms;
    private int bossRoomsCount;
    public int roomsCount;
    private Vector3 longestDistance;
	...
	void Start () {
        ...
        bossRoomsCount =targetBossRooms;
        StartGeneration();
    ...
    private void GenerateNextRoom() {
        ...
        if (bossRoomsCount != 0 && roomsCount > (targetRooms /3)){
            for (int i = 0; i < data.sets[dungeonSet].bosses.Count; i++)
            {
                possibleRooms.Add(data.sets[dungeonSet].bosses[i]);
            }
        }
        ...
        if (!roomIsGood) {
            ...
            roomsCount++;
            if (newRoom.tag.Equals("BossRooms")){
                bossRoomsCount -= 1;
                Debug.Log("BossRoom No"+roomsCount);
            }else if (newRoom.tag.Equals("EndRooms")){
                if (longestDistance.magnitude < newRoom.transform .position.magnitude){
                    longestDistance = newRoom.transform.position;
                    Debug.Log("Distance: " + longestDistance.magnitude);
                } 
            }
